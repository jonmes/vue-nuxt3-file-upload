  ...
  >
    ...
    <div :class="cx('buttonbar')" v-bind="ptm('buttonbar')">
      ...
      >
        <span
          ...
          @blur="onBlur"
          tabindex="0"
          v-bind="ptm('chooseButton')"
        >
          <slot name="chooseicon" :class="cx('chooseIcon')">
            <component
              :is="chooseIcon ? 'span' : 'PlusIcon'"
              :class="[cx('chooseIcon'), chooseIcon]"
              aria-hidden="true"
              v-bind="ptm('chooseIcon')"
            />
          </slot>
          <span
            :class="cx('chooseButtonLabel')"
            v-bind="ptm('chooseButtonLabel')"
        ...
        >
          <template #icon="iconProps">
            <slot name="uploadicon">
              <component
                :is="uploadIcon ? 'span' : 'UploadIcon'"
                :class="[iconProps.class, uploadIcon]"
                aria-hidden="true"
                v-bind="ptm('uploadButton')['icon']"
              />
            </slot>
          </template>
        </FileUploadButton>
        ...
        >
          <template #icon="iconProps">
            <slot name="cancelicon">
              <component
                :is="cancelIcon ? 'span' : 'TimesIcon'"
                :class="[iconProps.class, cancelIcon]"
                aria-hidden="true"
                v-bind="ptm('cancelButton')['icon']"
              />
            </slot>
          </template>
        </FileUploadButton>
    ...
    >
      <slot
        ...
        :files="files"
        :uploadedFiles="uploadedFiles"
        :removeUploadedFileCallback="removeUploadedFile"
        :removeFileCallback="remove"
        :progress="progress"
        :messages="messages"
      >
      ...
      <div
        v-if="$slots.empty && !hasFiles && !hasUploadedFiles"
        :class="cx('empty')"
        v-bind="ptm('empty')"
      >
  ...
  >
    ...
    >
      ...
      >
        <component
          :is="uploadIcon ? 'span' : 'UploadIcon'"
          :class="[cx('uploadIcon'), uploadIcon]"
          aria-hidden="true"
